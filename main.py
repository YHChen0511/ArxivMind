...
logger.info("LLM API Key Loaded: " + str((os.getenv("LLM_API_KEY"))))
logger.info("LLM API Base URL Loaded: " + str((os.getenv("LLM_API_BASE_URL"))))
# 初始化 OpenAI
client = AsyncOpenAI(
...
async def get_streaming_response(
    prompt: str, model: str = "gemini-3-pro-preview-thinking"
) -> str:
...
async def research_topic(request: SearchRequest):
    ...
    async def generate():
        try:
            ...
            yield json.dumps({"type": "progress", "step": "analyzing", "message": "Analyzing research intent..."}) + "\n"
...
You are a Senior Computer Vision Researcher with a "Problem-First" mindset.
            ...
            intent_content = await get_streaming_response(intent_prompt)
            ...
            yield json.dumps({"type": "progress", "step": "searching", "message": "Searching Arxiv..."}) + "\n"
            logger.info("Step 2: Executing multi-query search on Arxiv...")
            all_papers = {}  # Use dict for deduplication by entry_id

            def fetch_arxiv_results(search_query):
                try:
                    # Use a local client with specific settings
                    client = arxiv.Client(
                        page_size=30,
                        delay_seconds=3.0,
                        num_retries=3
            ...
            for i, query in enumerate(intent_data["queries"]):
                # Clean query
                clean_query = query.replace('"', "").replace("'", "")
                logger.info(f"Searching: {clean_query}")
                yield json.dumps({"type": "progress", "step": "searching", "message": f"Searching Arxiv for: {clean_query}..."}) + "\n"

                # Add explicit delay between queries to respect Arxiv API rate limits
                if i > 0:
                    await asyncio.sleep(3.0)

                results = await asyncio.to_thread(fetch_arxiv_results, clean_query)
            ...
            if not papers_list:
                yield json.dumps({"type": "result", "data": {"papers": [], "analysis": "No papers found. Try a broader query."}}) + "\n"
                return

            # --- Step 3: 智能重排序 (Reasoning Rerank) ---
            yield json.dumps({"type": "progress", "step": "reranking", "message": f"Reranking {len(papers_list)} papers..."}) + "\n"
            ...
            rerank_content = await get_streaming_response(rerank_prompt)
            ...
            final_result = {
                ...
                "analysis": {
                    ...
                    "keyTrends": intent_data.get(
                        "keywords", ["AI Research", "Computer Vision"]
                    ),
            ...
            for p in final_papers:
                p_copy = p.copy()
                if "raw_object" in p_copy:
                    del p_copy["raw_object"]
                clean_final_papers.append(p_copy)
            final_result["papers"] = clean_final_papers
